...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 30 16:44:14 2019
...
def Iteration(data,split_possibles,niveau,minNum):
    ...
    #if t==0 or t==len(data):    # if there are only C=0 or C=1 in our data, we do nothing
        #possible=False 
        #print("arret1")
    if len(split_possibles)==0:   #if all the possibilies of split have been used, we do nothing
        ...
        #print("arret3")
    if possible==True:           
        maxGINI=Gain(data,split_possibles[0][0])[0]
        ...
        for i in split_possibles:       #we calculate the max of the GINIsplit for the remaining split_possities
            ...
            if maxGINI==0 and (len(GINIsplit(data,i[0])[1])==0 or (len(GINIsplit(data,i[0])[2])==0)):
                maxGINI=a
                split_retenu=i
            if a>maxGINI:
                maxGINI=a
        ...
        res[niveau]=(split_retenu[0],Gain(data,split_retenu[0])[0],GINIsplit(data,split_retenu[0])[3])   #we append our result to our dictionnary
        data1=Gain(data,split_retenu[0])[1]                                 #we do again our function for the two sons
        data2=Gain(data,split_retenu[0])[2]
        if maxGINI!=0.0 or (len(GINIsplit(data,split_retenu[0])[1])!=0 and len(GINIsplit(data,split_retenu[0])[2])!=0):
